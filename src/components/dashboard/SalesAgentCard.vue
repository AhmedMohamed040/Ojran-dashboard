...
export default {
  ...
  computed: {
    ...mapGetters('Sales', ['getAgentSales']),
    ...mapGetters('PotentialMerchant', [
      'getPotentialMerchantList'
    ]),

    currentMonth() {
      return this.getAgentSales.currentMonthSales
    },
    lastMonth() {
      return this.getAgentSales.previousMonthSales
    },
    chartOptions() {
  ...
  mounted() {
    this.fetchPotentialMerchant().then((res) => {
      this.merchant = res.data.model[0]
    })
    setTimeout(()=> {
      this.getData()
    }, 1000)
  ...
  methods: {
    ...
    ...mapActions('Sales', ['fetchAgentSales']),
    setDailySales() {
      const days = []
      const sales = []

      for (let s = 0; s < this.getAgentSales.dailySales.length; s++) {
        days.unshift(this.getAgentSales.dailySales[s].day)
        sales.unshift(this.getAgentSales.dailySales[s].sales.toFixed(2))
      }
      this.days = days
    ...
    generateChart() {
      ...
      const salesOfLast7Days = last7Days.map((day) => {
        const sale = this.getAgentSales.dailySales.find((s) => moment(s.day, 'DD').format('MMMM Do, YYYY') === day)
    ...
    this.fetchAgentSales(this.getPotentialMerchantList).finally(() => {
      this.setDailySales()
      this.generateChart()
    })
     }
