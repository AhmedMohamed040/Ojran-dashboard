<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <div class="display-1">{{ $t('geography.countries') }}</div>
          ...
          <div class="d-flex justify-space-between w" style="width: 100%;">
            <div class="d-flex align-center" >
              <v-text-field
                ...
                outlined
                hide-details
                @input="fetchCountries(searchFilter)"
              ></v-text-field>
              <v-btn
                color="primary"
                class="mx-2"
          ...
          <template v-slot:default="dialog">
            <v-card>
              <v-toolbar color="red" dark>{{
                $t('geography.deleteCountry')
              }}</v-toolbar>
              <v-card-text>
                <h4 class="my-3 red--text">
                  {{
                    $t(
                      'geography.AreYouSureYouWantToDeleteThisCountry'
                    )
                  }}
                </h4>
              </v-card-text>
              <v-card-actions class="justify-end">
                <v-btn
                  ...
                  dark
                  @click="
                    deleteThisCountry(item.id), (dialog.value = false)
                  "
                >
                  {{ $t('common.delete') }}
                </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        {
          text: this.$i18n.t('geography.countries'),
          disabled: false,
          href: '#'
        },
        {
  ...
  computed: {
    ...mapGetters('GeographyCrud', [
      'getCountriesList'
    ]),
    formTitle() {
      return this.editedIndex === -1
        ? this.$i18n.t('geography.createCountry')
        : this.$i18n.t('geography.updateCountry')
    }
  },
