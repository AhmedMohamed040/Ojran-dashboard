<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <div class="display-1">{{ $t('menu.GiftCards') }}</div>
            ...
            <div class="d-flex justify-space-between w" style="width: 100%;">
              <div class="d-flex align-center" >
                <v-text-field
                  ...
                  outlined
                  hide-details
                  @input="fetchGiftCards(searchFilter)"
                ></v-text-field>
                <v-btn
                  color="primary"
                  class="mx-2"
            ...
            <template v-slot:default="dialog">
              <v-card>
                <v-toolbar color="red" dark>{{
                  $t('GiftCard.deleteGiftCard')
                }}</v-toolbar>
                <v-card-text>
                  <h4 class="my-3 red--text">
                    {{ $t('GiftCard.AreYouSureYouWantToDeleteThisGiftCard') }}
                  </h4>
                </v-card-text>
                <v-card-actions class="justify-end">
                  <v-btn
                    color="red"
                    dark
                    @click="deleteThisGiftCard(item.id), (dialog.value = false)"
                  >
                    {{ $t('common.delete') }}
                  </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        {
          text: this.$i18n.t('menu.GiftCards'),
          disabled: false,
          href: '#'
        },
        {
  ...
  computed: {
    ...mapGetters('GiftCard', ['getGiftCardList', 'getFilter', 'getLastPage']),
    formTitle() {
      return this.editedIndex === -1
        ? this.$i18n.t('GiftCard.createGiftCard')
        : this.$i18n.t('GiftCard.updateGiftCard')
    }
  },
  ...
  methods: {
    ...
    ...mapMutations('GiftCard', ['setFilter']),
    handlingRoute(event) {
    ...
    deleteThisGiftCard(id) {
      this.deleteGiftCard(id).then(() => {
        this.fetchGiftCards()
      })
    }
  },
  watch: {
    $route: {
      async handler(route) {
        ...
        this.$store
          .dispatch('GiftCard/fetchGiftCards', route.query)
          .then(() => {
            this.loading = false
          })
      },
