<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <div class="display-1">{{ $t('geography.Nationalities') }}</div>
            ...
            <div class="d-flex justify-space-between w" style="width: 100%;">
              <div class="d-flex align-center" >
                <v-text-field
                  ...
                  outlined
                  hide-details
                  @input="fetchNationalitys(searchFilter)"
                ></v-text-field>
                <v-btn
                  color="primary"
                  class="mx-2"
                  @click="
                    ...
                    fetchNationalitys(searchFilter)
                  "
                >
                  {{ $t('common.showAll') }}
                </v-btn>  
            ...
            <template v-slot:default="dialog">
              <v-card>
                <v-toolbar color="red" dark>{{
                  $t('Nationality.deleteNationality')
                }}</v-toolbar>
                <v-card-text>
                  <h4 class="my-3 red--text">
                    {{
                      $t('Nationality.AreYouSureYouWantToDeleteThisNationality')
                    }}
                  </h4>
                </v-card-text>
                <v-card-actions class="justify-end">
                  <v-btn
                    ...
                    dark
                    @click="
                      deleteThisNationality(item.id), (dialog.value = false)
                    "
                  >
                    {{ $t('common.delete') }}
                  </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        {
          text: this.$i18n.t('geography.Nationalities'),
          disabled: false,
          href: '#'
        },
        {
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1
        ? this.$i18n.t('Nationality.addNationality')
        : this.$i18n.t('Nationality.updateNationality')
    }
  },

  methods: {
    ...mapActions('Nationalities', [
      'fetchNationalitys',
      'postNationality',
      'updateNationality',
    ...
    ...mapMutations('Nationalities', ['setFilter']),
    handlingRoute(event) {
    ...
    deleteThisNationality(id) {
      this.deleteNationality(id).then(() => {
        this.fetchNationalitys()
      })
    }
  },
  watch: {
    $route: {
      async handler(route) {
        ...
        this.$store
          .dispatch('Nationalities/fetchNationalitys', route.query)
          .then(() => {
            this.loading = false
          })
      },
