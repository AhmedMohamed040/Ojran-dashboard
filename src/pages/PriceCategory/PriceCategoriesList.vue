<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center py-3">
      <div>
        <div class="display-1">{{ $t('menu.PriceCategories') }}</div>
            ...
            <div class="d-flex justify-space-between w" style="width: 100%;">
              <div class="d-flex align-center" >
                <v-text-field
                  ...
                  outlined
                  hide-details
                  @input="fetchPriceCategorys(searchFilter)"
                ></v-text-field>
                <v-btn
                  color="primary"
                  class="mx-2"
                  @click="
                    ...
                    fetchPriceCategorys(searchFilter)
                  "
                >
                  {{ $t('common.showAll') }}
                </v-btn>  
              ...
              <v-dialog v-model="dialog" max-width="700">
                <template v-slot:activator="{ on, attrs }">
                  <v-btn
                    ...
                    v-on="on"
                  >
                    {{ $t('PriceCategory.createPriceCategory') }}
                  </v-btn>
                </template>
                <v-card>
            ...
            <template v-slot:default="dialog">
              <v-card>
                <v-toolbar color="red" dark>{{
                  $t('PriceCategory.deletePriceCategory')
                }}</v-toolbar>
                <v-card-text>
                  <h4 class="my-3 red--text">
                    {{
                      $t(
                        'PriceCategory.AreYouSureYouWantToDeleteThisPriceCategory'
                      )
                    }}
                  </h4>
                </v-card-text>
                <v-card-actions class="justify-end">
                  <v-btn
                    ...
                    dark
                    @click="
                      deleteThisPriceCategory(item.id), (dialog.value = false)
                    "
                  >
                    {{ $t('common.delete') }}
                  </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        {
          text: this.$i18n.t('menu.PriceCategories'),
          disabled: false,
          href: '#'
        },
        {
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1
        ? this.$i18n.t('PriceCategory.createPriceCategory')
        : this.$i18n.t('PriceCategory.updatePriceCategory')
    }
  },

  methods: {
    ...mapActions('PriceCategory', [
      'fetchPriceCategorys',
      'postPriceCategory',
      'updatePriceCategory',
      'changePriceCategoryStatus',
      'deletePriceCategory'
    ]),
    ...mapMutations('PriceCategory', ['setFilter']),
    handlingRoute(event) {
    ...
    blockUser(id) {
      this.changePriceCategoryStatus({
        id: id,
        status: false
      })
    },
    ...
    deleteThisPriceCategory(id) {
      this.deletePriceCategory(id).then(() => {
        this.fetchPriceCategorys()
      })
    }
  },
  watch: {
    $route: {
      async handler(route) {
        ...
        this.$store
          .dispatch('PriceCategory/fetchPriceCategorys', route.query)
          .then(() => {
            this.loading = false
          })
      },
