    ...
    <div v-if="!loading">
      ...
      >
        ...
        <template v-slot:[`item.invoiceId`]="{ item }">
          <copy-label :text="item.invoiceId" />
        </template>
...
export default {
  ...
  computed: {
    ...mapGetters('Moderators', ['getBillsList', 'getLastPage', 'getCustomer'])
  },

  methods: {
    ...mapActions('Moderators', ['fetchBills', 'fetchCustomer']),
    ...
    emptyFilters() {
      ...
      this.searchFilter.UserID = this.$route.params.singlecustomer_id
      this.fetchBills(this.searchFilter).finally(() => {
        this.isLoading = false
      })
    }
  ...
  watch: {
    $route: {
      async handler(route) {
        ...
        this.searchFilter.UserID = route.params.singlecustomer_id
        this.searchFilter.PageNumber = route.query.page
        this.$store
          .dispatch('Moderators/fetchBills', this.searchFilter)
          .then(() => {
            this.loading = false
          })
      },
 ...
 //   this.fetchCustomer(this.$route.params.singlecustomer_id)
   // console.log(this.page, this.getCustomer)
  }
}
</script>
