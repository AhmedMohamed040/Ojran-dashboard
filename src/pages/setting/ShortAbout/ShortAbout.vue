<template>
  <div class="d-flex flex-column flex-grow-1 ma-0">
    <div class="d-flex align-center py-3">
      <div>
        ...
        <div class="display-1">{{ $t('setting.shortAbout') }}</div>
 ...
 -->
        ...
        <v-card-actions>
          <v-btn color="primary" class="mx-2" @click="save(getShortAbout)">
            {{ $t("common.save") }}
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      footer: [
        ...
        {
          text: this.$i18n.t('setting.shortAbout')
        }
      ],
      error: false,
      snackbar: false,
  ...
  computed: {
    ...mapGetters('AboutUs', ['getShortAbout']),
    getAboutUsText() {
      return this.getShortAbout
    },
    dir() {
      if (this.tab == 0) {
        return 'rtl'
  ...
  methods: {
    ...mapActions('AboutUs', ['fetchShortAbout', 'changeShortAbout']),
    save(item) {
      if (this.tab == 0) {
        this.changeShortAbout({
          id: item.id,
          Value: item.value,
